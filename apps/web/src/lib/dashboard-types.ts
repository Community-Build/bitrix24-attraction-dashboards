...
export interface DealAnalysisTimelineItem { id: string; sourceEntityId: string; kind: 'call' | 'task_created' | 'task_completed' | 'meeting' | 'meeting_date_changed' | 'conversion_event_visit' | 'message'; occurredAt: string; title: string; detail: string | null; comment: string | null; createdAt: string | null; deadlineAt: string | null; completedAt: string | null; direction: 'incoming' | 'outgoing' | 'unknown' | null; durationSeconds: number | null; stageName: string | null }
...
export interface DealAnalysisDetail { row: DealAnalysisRow; timeline: DealAnalysisTimelineItem[]; stageHistory: Array<{ id: string; stageId: string; stageName: string; enteredAt: string }>; callInsights: DealAnalysisCallInsight[] | null; sensitiveContentAvailable: boolean }
