...
| Messenger analysis and Activities reader | attraction | Activities messenger section, `/api/messenger-messages/summary`, `/read`, `/attachment`, `/collect`, `messenger-message-collection.ts` | Current attraction deals, manager whitelist, Bitrix Open Lines | Leader-only manual collection; summary is aggregate-only, bounded reader text and validated attachments are transient and no-store. |
...
| Messenger message analysis and reader | attraction | `/api/messenger-messages/summary`, `/read`, `/attachment`, `/collect`, messenger collection service | Bitrix24 Open Lines and Disk | none; raw text and proxied files are transient only |
